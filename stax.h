...
//
// A usual CPU takes up around 1,000-2,000 bytes of memory (3,000+ w/ memory enable)
...
struct cpu_settings_t
{
  ...
  int max_memory_allocation_pool; // The max amount of memory that can be
                                  // allocated from anonymous requests. (Note:typedef int (*ivtfn32) (CPU*);
                                  // -1 disables this)                        typedef int byte;
  bool silent;
};
