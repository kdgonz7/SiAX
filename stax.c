...
void
r_free_list (RollocFreeList *freelist)
{
  assert (freelist);
...
void
r_free_node (RollocFreeList *list, RollocNode *node)
{
  assert (node);
...
int
cpu_hash (const char *N, size_t m)
{
  int r = 1;
...
//      instruction.
void
ivt_map (vivt32 *table, ivtfn32 function, const char *instruction_name,
         bool dv)
{
  assert (table);
...
void
ord_append (Order *order, byte *data, int size)
{
  assert (order);
  ...
  if (!order->data)
    ...
    {
      order->data
          = realloc (order->data, (order->data_size + size) * sizeof (byte));
      order->data_size += size;
      p = order->data_size;
    }
...
CPU *
vcpu (struct cpu_settings_t settings)
{
...
void
cpu_exe (CPU *vcpu, byte *info, size_t size)
{
  assert (vcpu);
  assert (info);
...
int
test_cpu_instruction_hash (void)
{
  printf ("hash1: 'DIE': %d\n", cpu_hash ("DIE", 101));
  printf ("hash1: 'DIE2': %d\n", cpu_hash ("DIE2", 101));
  printf ("hash1: 'DIE3': %d\n", cpu_hash ("DIE3", 101));
